...
function measureExportFeatureMapLabelTextWidth(text: string, fontSize: number) {
  ...
  context.font = `700 ${fontSize}px ${getDromapRuntimeFontFamily()}`;
...
export function ExportLeafletPreview({
  ...
  const dataAttributionText = useMemo(
    () =>
      getDromapExportDataAttributionText(
        features,
        renderableGeoJsonLayers,
      ),
    [features, renderableGeoJsonLayers],
  );

  const previewAttributionHtml = useMemo(
  ...
  return (
    ...
    >
      ...
      {previewAttributionHtml || dataAttributionText ? (
        <div
          className={[
            "pointer-events-auto absolute bottom-1 right-1 max-w-[70%] px-1 py-px text-right text-[7px] leading-tight",
            basemap.id === "ign-satellite"
              ? "text-slate-700"
              : "text-slate-700/65",
          ].join(" ")}
          style={{
            textShadow:
              "0 0 2px rgba(255,255,255,.92), 0 0 4px rgba(255,255,255,.72)",
          }}
        >
          {previewAttributionHtml ? (
            <span
              dangerouslySetInnerHTML={{
                __html: previewAttributionHtml,
              }}
            />
          ) : null}
          {previewAttributionHtml && dataAttributionText
            ? " · "
            : null}
          {dataAttributionText ? <span>{dataAttributionText}</span> : null}
        </div>
      ) : null}
    </div>
  );
