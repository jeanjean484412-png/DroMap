...
export function GeoJsonLibraryBrowser({
  ...
  const libraryLocked =
    runtime.enabled && !runtime.capabilities.canUseGeoJsonLibrary;
  ...
  async function downloadPreparedGeoBoundaries(
    ...
    const sourceLicense =
      metadata.boundaryLicense || "CC BY 4.0 (geoBoundaries gbOpen)";
    const sourceAttribution = ["geoBoundaries", metadata.boundarySource]
    ...
    return (
      <section className="rounded-2xl border border-teal-200 bg-white p-4 shadow-sm">
        <div className="flex items-start gap-3">
          <span className="mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-teal-100 text-teal-700">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm leading-relaxed text-slate-600">
              La bibliothèque DroMap de jeux GeoJSON prêts à l’emploi est
              disponible après création d’un compte. L’import d’un fichier
              GeoJSON depuis ton ordinateur reste disponible séparément.
            </p>
            <button
              type="button"
              onClick={() =>
                runtime.requestRestriction({
                  title: "Bibliothèque GeoJSON réservée aux comptes DroMap",
                  description:
                    "Crée un compte gratuit pour accéder à la bibliothèque de données GeoJSON DroMap. Ton projet actuel reste conservé.",
                })
              }
              className="mt-3 inline-flex items-center gap-2 rounded-xl border border-teal-300 bg-teal-600 px-3 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-teal-500"
            >
              <LockIcon className="h-3.5 w-3.5" />
              Créer un compte pour accéder à la bibliothèque
            </button>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="rounded-2xl border border-teal-200 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <label className="shrink-0 text-xs font-semibold text-teal-900">
          Précision d’import
          <select
            value={precisionMode}
            ...
            className="mt-1 block w-full min-w-36 rounded-lg border border-teal-200 bg-white px-2 py-2 text-xs font-medium text-slate-900"
          >
      ...
      <div className="mt-5 border-t border-slate-200 pt-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_15rem]">
          <label className="relative block">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100"
            />
          </label>
          ...
          <label>
            ...
            <select
              ...
              className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-medium text-slate-900 outline-none transition focus:border-teal-500 focus:ring-2 focus:ring-teal-100"
            >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap gap-1.5 text-[10px] font-semibold">
                        ...
                        <span className="rounded-full bg-teal-50 px-2 py-0.5 text-teal-700 ring-1 ring-teal-100">
        ...
        {visibleEntries.length < filteredEntries.length ? (
          <button
            ...
            className="mt-3 w-full rounded-xl border border-teal-200 bg-teal-50 px-3 py-2.5 text-xs font-bold text-teal-800 transition hover:bg-teal-100"
          >
      ...
      {heavyWithoutWorkspacePrompt && typeof document !== "undefined"
        ? createPortal(
            ...
            >
              <section
                className="w-full max-w-lg overflow-hidden rounded-2xl border border-amber-300 bg-white shadow-lg"
                onMouseDown={(event) => event.stopPropagation()}
              >
      ...
      {status ? (
        <div className="mt-4 rounded-xl border border-teal-100 bg-teal-50 px-3 py-2.5 text-xs leading-relaxed text-teal-950">
          {status}
        </div>
      ) : null}
    </section>
