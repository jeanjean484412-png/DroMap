...
function restoreMapCursorIfSelectionToolIsActive() {
  ...
  const activeTool = useEditorToolStore.getState().activeTool;
...
export function ActiveToolOutsideWorkspaceClickGuard() {
  useEffect(() => {
    ...
    const unsubscribeToolStore = useEditorToolStore.subscribe(
      (state, previousState) => {
        if (
    ...
    const handleDocumentClick = (event: MouseEvent) => {
      ...
      if (useEditorModeStore.getState().currentMode !== "edit") {
        return;
      }

      const toolStore = useEditorToolStore.getState();
