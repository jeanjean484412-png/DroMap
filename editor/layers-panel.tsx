...
export function LayersPanel({
  ...
  const replaceFeatures = useEditorFeaturesStore((state) => state.replaceFeatures);
  ...
  const renameLayer = useEditorLayersStore((state) => state.renameLayer);
  const deleteLayer = useEditorLayersStore((state) => state.deleteLayer);
  const setActiveLayerId = useEditorLayersStore((state) => state.setActiveLayerId);
  const toggleLayerVisibility = useEditorLayersStore(
    (state) => state.toggleLayerVisibility,
  );
  const setLayerOpacity = useEditorLayersStore((state) => state.setLayerOpacity);
  const toggleLayerLocked = useEditorLayersStore((state) => state.toggleLayerLocked);
  const moveLayer = useEditorLayersStore((state) => state.moveLayer);
  const loadSavedLayersFromStorage = useEditorLayersStore(
    (state) => state.loadSavedLayersFromStorage,
  );
  const saveLayerToLibrary = useEditorLayersStore(
    (state) => state.saveLayerToLibrary,
  );
  const renameSavedLayer = useEditorLayersStore((state) => state.renameSavedLayer);
  const deleteSavedLayer = useEditorLayersStore((state) => state.deleteSavedLayer);

  const geoJsonLayers = useEditorGeoJsonLayersStore((state) => state.geoJsonLayers);
  const renameGeoJsonLayer = useEditorGeoJsonLayersStore(
    (state) => state.renameGeoJsonLayer,
  );
  const removeGeoJsonLayer = useEditorGeoJsonLayersStore(
    (state) => state.removeGeoJsonLayer,
  );
  const toggleGeoJsonLayerVisibility = useEditorGeoJsonLayersStore(
    (state) => state.toggleGeoJsonLayerVisibility,
  );
  const setGeoJsonLayerOpacity = useEditorGeoJsonLayersStore(
    (state) => state.setGeoJsonLayerOpacity,
  );
  const setGeoJsonLayerPrecisionMode = useEditorGeoJsonLayersStore(
    (state) => state.setGeoJsonLayerPrecisionMode,
  );
  const toggleGeoJsonLayerLocked = useEditorGeoJsonLayersStore(
    (state) => state.toggleGeoJsonLayerLocked,
  );
  const moveGeoJsonLayer = useEditorGeoJsonLayersStore(
    (state) => state.moveGeoJsonLayer,
  );
  const updateGeoJsonLayerStyle = useEditorGeoJsonLayersStore(
  ...
  const saveGeoJsonLayerToLibrary = useEditorGeoJsonLayersStore(
  ...
  function renderDialog() {
    ...
    return createPortal(
      ...
      >
        <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-4 text-sm shadow-lg">
          ...
          {isNameDialog ? (
            <label className="mt-4 block text-xs font-medium text-slate-700">
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 outline-none transition focus:border-teal-400 focus:ring-2 focus:ring-teal-100"
              />
            </label>
          ) : null}
  ...
  if (!isOpen && !embedded) {
    return (
      <button
        ...
        className="group absolute right-4 top-[5.25rem] z-[1000] flex min-w-56 cursor-pointer items-center justify-between gap-3 rounded-2xl border-2 border-teal-200 bg-white/95 px-3 py-2.5 text-left text-sm text-slate-900 shadow-lg backdrop-blur transition hover:border-teal-400 hover:bg-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-teal-100"
      >
        <span className="flex min-w-0 items-center gap-2.5">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-teal-600 text-white shadow-sm transition group-hover:bg-teal-500">
            <svg
              aria-hidden="true"
              width="19"
              height="19"
          ...
          <span className="min-w-0">
            <span className="block text-[11px] font-black uppercase tracking-wide text-teal-700">
              Calques
            </span>
        ...
        <span className="shrink-0 rounded-lg bg-teal-50 px-2 py-1 text-[11px] font-black text-teal-700 transition group-hover:bg-teal-100">
          Gérer →
        </span>
      </button>
    );
    ...
    <>
      <aside
        ...
        className={
          embedded
            ...
            : "absolute right-4 top-[5.25rem] flex max-h-[72vh] w-[25rem] flex-col overflow-hidden rounded-2xl border border-black/10 bg-white/95 text-sm shadow-lg backdrop-blur"
        }
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
          ...
          <section className="rounded-2xl border border-teal-200 bg-teal-50 p-3 shadow-sm">
            <button
              type="button"
              onClick={() => {
                if (!capabilities.canSaveLayersToLibrary) {
              ...
              className="group flex w-full items-center justify-between gap-3 rounded-xl border border-teal-200 bg-white px-3 py-3 text-left shadow-sm transition hover:border-teal-400 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-teal-100"
            >
              <span className="flex min-w-0 items-center gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-600 text-white shadow-sm transition group-hover:bg-teal-500">
                  <svg
                    aria-hidden="true"
                    width="21"
                    height="21"
              ...
              <span className="shrink-0 rounded-lg bg-teal-50 px-2.5 py-1.5 text-xs font-black text-teal-700 transition group-hover:bg-teal-100">
