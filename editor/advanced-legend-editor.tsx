            ...
            >
              ...
              <button
                ...
                className={`flex w-full items-center rounded-xl border text-left transition ${
                  selected
                    ? "border-teal-500 bg-teal-50/90 ring-2 ring-teal-200"
        ...
        {options.map((option) => {
          ...
          return (
            <button
              ...
              className={`flex h-16 flex-col items-center justify-center rounded-lg border bg-white p-1 text-[9px] font-medium leading-tight hover:border-teal-400 ${
                value === option.id
...
export function AdvancedLegendEditor({
  ...
  const globalSymbolSize = useEditorExportStore(
  ...
  const setLegendItemGap = useEditorExportStore(
    (state) => state.setLegendItemGap,
  );
  const setLegendLabelGap = useEditorExportStore(
    (state) => state.setLegendLabelGap,
  );
  const setLegendLabelLineHeight = useEditorExportStore(
    (state) => state.setLegendLabelLineHeight,
  );
  const setLegendSectionGap = useEditorExportStore(
    (state) => state.setLegendSectionGap,
  );
  const setLegendMapBorderEnabled = useEditorExportStore(
    (state) => state.setLegendMapBorderEnabled,
  );
  const setLegendMapBorderColor = useEditorExportStore(
    (state) => state.setLegendMapBorderColor,
  );
  const setLegendMapBorderWidth = useEditorExportStore(
    (state) => state.setLegendMapBorderWidth,
  );
  const setLegendMapBorderRadius = useEditorExportStore(
    (state) => state.setLegendMapBorderRadius,
  );
  const setLegendMapPadding = useEditorExportStore(
    (state) => state.setLegendMapPadding,
  );
  const setLegendSymbolOverride = useEditorExportStore(
    (state) => state.setLegendSymbolOverride,
  );
  const setLegendSymbolOverrides = useEditorExportStore(
    (state) => state.setLegendSymbolOverrides,
  );
  const resetLegendSymbolOverride = useEditorExportStore(
  ...
  const addCustomLegendEntry = useEditorExportStore(
    (state) => state.addCustomLegendEntry,
  );
  const removeCustomLegendEntry = useEditorExportStore(
      ...
      <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-5 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-xl border border-teal-300 bg-teal-50 px-4 py-2 text-sm font-semibold text-teal-800 hover:bg-teal-100 disabled:cursor-wait disabled:opacity-70"
                  ...
                  selectedEntry.representativeFeature.properties.symbol
                    ...
                    <div className="rounded-xl border border-teal-200 bg-teal-50 p-4">
                      <div className="flex items-center gap-4">
                        ...
                        />
                        <div>
                          <strong className="block text-sm text-teal-950">
