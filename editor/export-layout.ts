...
function getEstimatedMapLegendHeight(
  ...
  const titleLineCount = titleText
    ...
    : 0;
  const titleHeight = titleLineCount > 0
    ? Math.ceil(titleLineCount * appearance.titleFontSize * 1.16) +
      Math.max(16, Math.round(appearance.itemFontSize * 0.65))
    : 0;
  ...
  return Math.max(1, Math.ceil(contentInset * 2 + titleHeight + contentHeight));
}
...
function createMapOverlayLegendRect(
  ...
  const titleWidth = content?.title
    ? estimateLegendTextWidth(content.title, appearance.titleFontSize, 700)
    : 0;
  const preferredWidth = Math.ceil(
    horizontalInset * 2 +
      Math.max(titleWidth, longestSectionWidth, labelOffset + longestEntryWidth) +
      16,
  );
...
function createAutoExportLayout(
  ...
  if (legendPosition === "bottom" || legendPosition === "top") {
    ...
    const mapRect = {
      x: EXPORT_PAGE_PADDING,
      y: isTop
        ? EXPORT_PAGE_PADDING + legendHeight + EXPORT_LAYOUT_GAP
        : EXPORT_PAGE_PADDING,
      width: mapSize.width,
      height: mapSize.height,
    };
    ...
    return withMapRenderScale(
      {
        ...
        legendRect: {
          ...
          y: isTop
            ? EXPORT_PAGE_PADDING
            : EXPORT_PAGE_PADDING + mapSize.height + EXPORT_LAYOUT_GAP,
          width: mapSize.width,
          height: legendHeight,
        },
      },
...
function createFixedExportLayout(
  ...
  if (legendPosition === "bottom" || legendPosition === "top") {
    ...
    const mapAreaRect: ExportCanvasRect = {
      x: contentRect.x,
      y: isTop
        ? contentRect.y + legendHeight + EXPORT_LAYOUT_GAP
        : contentRect.y,
    ...
    return withMapRenderScale(
      {
        ...
        legendRect: {
          ...
          y: isTop
            ? contentRect.y
            : canvasHeight - EXPORT_PAGE_PADDING - legendHeight,
          width: contentRect.width,
          height: legendHeight,
        },
      },
