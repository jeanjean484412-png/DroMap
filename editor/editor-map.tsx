...
import { useEditorWorldSnapshotStore } from "@/stores/editor-world-snapshot";
...
function MapViewStateBridge() {
  ...
  const updateCurrentView = () => {
    ...
    useEditorMapViewStore.getState().setCurrentView({
    ...
    firstFrameId = window.requestAnimationFrame(() => {
      secondFrameId = window.requestAnimationFrame(() => {
        timeoutId = window.setTimeout(() => {
          const pendingView = useEditorMapViewStore
            .getState()
...
function MapPresentationReadyBridge({
  ...
  useEffect(() => {
    ...
    const prerequisitesAreSettled = () => {
      ...
      const mapViewState = useEditorMapViewStore.getState();
      ...
      const mode = useEditorModeStore.getState().currentMode;
      ...
      if (usesWorldSnapshot) {
        const snapshotStatus = useEditorWorldSnapshotStore.getState().status;
...
export default function EditorMap({ onPresentationReady }: EditorMapProps = {}) {
  ...
  const worldSnapshotBasemapId = useEditorWorldSnapshotStore(
    (state) => state.basemapId,
  );
  const worldSnapshotStatus = useEditorWorldSnapshotStore(
    (state) => state.status,
  );
      ...
      >
        {shouldRenderLiveBasemap && basemap.kind === "tile" ? (
          <TileLayer
            ...
            // Ne jamais forcer minNativeZoom sur le niveau verrouillé. Au remontage
            // d'un projet, MapContainer démarre brièvement au zoom initial (6) avant
            // que la vue sauvegardée soit restaurée. Avec minNativeZoom=18/19,
            // Leaflet tente alors de couvrir un viewport de zoom 6 avec des tuiles
            // natives de zoom 18/19, ce qui peut créer une quantité gigantesque de
            // tuiles et épuiser la mémoire. maxNativeZoom suffit à verrouiller le
            // niveau de détail maximal une fois la vue restaurée.
