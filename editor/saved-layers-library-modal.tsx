...
export function SavedLayersLibraryModal({
  ...
  return createPortal(
    <div
      className="fixed inset-0 flex items-center justify-center bg-slate-950/45 p-2 backdrop-blur-sm sm:p-4"
    ...
    >
      <section
        ...
        className="flex h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] flex-col overflow-hidden rounded-2xl border border-white/60 bg-white shadow-lg sm:h-[min(780px,calc(100dvh-2rem))] sm:w-[min(1180px,calc(100vw-2rem))]"
        onMouseDown={(event) => {
          event.stopPropagation();
          setModalZIndex(bringFloatingPanelToFront());
        }}
      >
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-slate-50/90 px-3 py-3 sm:gap-4 sm:px-5 sm:py-4">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 overflow-y-auto md:grid-cols-[minmax(280px,0.38fr)_minmax(0,0.62fr)] md:overflow-hidden">
          <aside className="flex min-h-0 flex-col border-r border-slate-200 bg-slate-50/70 p-4">
            <label className="block">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-teal-400 focus:ring-4 focus:ring-teal-100"
              />
            </label>
                ...
                <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 shadow-sm">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      className="rounded-xl bg-teal-600 px-4 py-2 text-xs font-black text-white shadow-sm transition hover:bg-teal-500 disabled:cursor-not-allowed disabled:bg-slate-300"
                    >
                      {isSelectedEntryApplied
                        ? "Déjà ajouté"
                        : mode === "select"
