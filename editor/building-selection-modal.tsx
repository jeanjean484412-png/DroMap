...
export function getBuildingSelectionFeatureId(
  feature: BuildingSelectionFeature,
...
function SelectionLayer({
  ...
  useEffect(() => {
    ...
    features.forEach((feature, index) => {
      idByFeature.set(feature, getBuildingSelectionFeatureId(feature, index));
    });
...
export function BuildingSelectionModal({
  ...
  const canConfirm =
    ...
    (allowEmptyConfirm && initialSelectedIds.length > 0);
  ...
  const applyAcceptedNameProposals = () => {
    ...
    setWorkingFeatures((current) =>
      current.map((feature, index) => {
        const id = getBuildingSelectionFeatureId(feature, index);
  ...
  const modal = (
    <div
      ...
      data-dromap-building-selection-modal="true"
    >
      <header className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-200 bg-white px-5 py-3 shadow-sm">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs text-slate-600">
            {introText ??
              "Clique sur les bâtiments utiles. Les éléments orange seront ajoutés directement comme objets DroMap éditables individuellement."}
          </p>
        </div>
      ...
      <div className="relative min-h-0 flex-1">
        <MapContainer
          ...
          zoom={16}
          className="dromap-building-selection-map h-full w-full"
          style={{
        ...
        <div className="pointer-events-none absolute left-4 top-4 z-[1000] rounded-xl border border-slate-200 bg-white/95 px-3 py-2 text-xs text-slate-700 shadow-lg backdrop-blur">
          <div className="font-black text-slate-950">{areaLabel}</div>
        ...
        {nameProposals.length > 0 ? (
          <div className="absolute right-4 top-4 z-[1100] max-h-[calc(100%-2rem)] w-[26rem] max-w-[calc(100%-2rem)] overflow-auto rounded-2xl border border-teal-200 bg-white/97 p-4 text-xs shadow-lg backdrop-blur">
            ...
            <button
              ...
              className="mt-3 w-full rounded-lg bg-teal-600 px-3 py-2 font-black text-white hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
      ...
      <footer className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-white px-5 py-3 shadow-[0_-8px_24px_rgba(15,23,42,0.08)]">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {unnamedSelectedEntries.length > 0 && !isWholeZoneSelection ? (
            <button
              ...
              className="rounded-lg border border-teal-300 bg-teal-50 px-3 py-2 text-xs font-black text-teal-800 transition hover:bg-teal-100 disabled:cursor-wait disabled:opacity-60"
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {selectedFeatures.length === 0
              ? "Valider : aucun bâtiment"
              : `Valider ${selectedFeatures.length.toLocaleString("fr-FR")} bâtiment${selectedFeatures.length > 1 ? "s" : ""}`}
          </button>
        </div>
      </footer>
    </div>
