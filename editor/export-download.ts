...
import {
  ...
  getFeatureVisualScaleForStaticRender,
...
import type { DroMapLayer } from "@/stores/editor-layers";
import {
...
import { getDromapExportDataAttributionText } from "./export-attribution";
...
export type DownloadCanvasExportInput = {
  ...
  /**
   * Miniatures persistées uniquement : force la lecture du zoom de référence
   * enregistré sur chaque objet, sans dépendre du mode courant de l’éditeur.
   * La preview finale et les exports normaux laissent cette option absente.
   */
  useStoredFeatureVisualScale?: boolean;
  creatorAttribution?: {
    label: string;
...
function getExportPixelRatioForQuality(quality: ExportVisualQuality) {
  switch (quality) {
    case "very-high":
      return 4;
    case "high":
      return HIGH_EXPORT_PIXEL_RATIO;
    case "standard":
    default:
...
const DEFAULT_TILE_FETCH_CONCURRENCY = 18;
const SATELLITE_TILE_FETCH_CONCURRENCY = 10;
const TILE_FETCH_RETRY_DELAYS_MS = [0, 180, 500] as const;
...
/**
 * Canvas applique globalAlpha à chaque opération de dessin. Pour un marqueur
 * rempli semi-transparent, dessiner d'abord le remplissage puis le contour
 * cumule donc deux fois l'opacité sur le bord, contrairement au SVG/DOM où
 * l'opacité est appliquée au groupe complet. On compose le marqueur opaque
 * sur un petit canvas intermédiaire puis on applique l'opacité une seule fois.
 */
function drawMarkerSymbolOnCanvasWithGroupOpacity(
  ctx: CanvasRenderingContext2D,
  feature: DroMapFeature,
  centerX: number,
  centerY: number,
  size: number,
  options?: { strokeColor?: string; shadow?: boolean },
) {
  const opacity = clamp(
    Number(feature.properties?.style?.opacity ?? 1),
    0,
    1,
  );

  if (opacity >= 0.999 || typeof document === "undefined") {
    drawMarkerSymbolOnCanvas(ctx, feature, centerX, centerY, size, options);
    return;
  }

  // Les très grands figurés restent possibles sans créer un canvas
  // intermédiaire démesuré. Le chemin normal couvre largement les tailles
  // visibles usuelles tout en évitant une forte pression mémoire.
  const safeSize = Math.max(6, Math.min(4096, size));
  if (safeSize > 1536) {
    drawMarkerSymbolOnCanvas(ctx, feature, centerX, centerY, size, options);
    return;
  }

  const shadowMargin = options?.shadow === false
    ? 4
    : Math.max(12, safeSize * 0.18);
  const width = Math.max(1, Math.ceil(safeSize + shadowMargin * 2));
  const height = width;
...
function waitForTileRetry(delayMs: number) {
  if (delayMs <= 0) {
...
async function loadImageFromUrlWithRetry(url: string) {
  let lastError: unknown = new Error("Impossible de charger une tuile.");

  for (const delayMs of TILE_FETCH_RETRY_DELAYS_MS) {
    await waitForTileRetry(delayMs);
...
async function loadTileImage(
  ...
  try {
    return await loadImageFromUrlWithRetry(tileUrl);
  } catch (error) {
    // Sur le satellite, ne jamais remplacer ponctuellement une orthophoto par
    // un fond routier. Si une tuile HD reste indisponible après les tentatives,
    // drawTiles laisse apparaître une couche satellite de secours moins détaillée
    // sous la tuile manquante. Cela évite les carrés noirs sans mélanger les fonds.
    ...
    return loadImageFromUrlWithRetry(fallbackTileUrl);
  }
}
...
async function loadTilesAtZoom(
  points: WorkspaceBoundsPoints,
  basemap: DromapBasemapConfig,
  zoom: number,
) {
  ...
  const requestedConcurrency =
    basemap.id === "ign-satellite"
      ? SATELLITE_TILE_FETCH_CONCURRENCY
      : DEFAULT_TILE_FETCH_CONCURRENCY;
  const workerCount = Math.min(requestedConcurrency, coordinates.length);
  let nextCoordinateIndex = 0;

  await Promise.all(
    Array.from({ length: workerCount }, async () => {
      while (true) {
        const coordinateIndex = nextCoordinateIndex;
        ...
        const { tileX, tileY } = coordinates[coordinateIndex];

        try {
          const image = await loadTileImage(basemap, zoom, tileX, tileY);
          loadedTiles.push({
            image,
            tileX,
            tileY,
...
async function loadTiles(
  ...
  const { coverage, loadedTiles } = await loadTilesAtZoom(
    points,
    basemap,
    zoom,
  );
...
function drawLoadedTileLayer(
  ctx: CanvasRenderingContext2D,
  mapRect: ExportCanvasRect,
  coverage: ReturnType<typeof getTileCoverage>,
...
async function drawTiles(
  ...
  const missingTileCount = Math.max(0, coverage.tileCount - loadedTiles.length);
  let satelliteFallbackLayer:
    | Awaited<ReturnType<typeof loadTilesAtZoom>>
    | null = null;
  ...
  if (
    ...
    // Très haute qualité demande beaucoup plus de tuiles natives (+2 niveaux).
    // Quelques réponses IGN peuvent alors échouer ponctuellement. On charge une
    // mosaïque satellite moins détaillée sous la couche HD : les tuiles HD qui
    // réussissent la recouvrent, et une éventuelle tuile manquante laisse voir
    // l'orthophoto de secours au lieu du fond #111827 (carré noir).
    const fallbackZoom = Math.max(0, zoom - 2);
    satelliteFallbackLayer = await loadTilesAtZoom(
      points,
      basemap,
      fallbackZoom,
    );
  ...
  if (satelliteFallbackLayer) {
    drawLoadedTileLayer(
      ctx,
      mapRect,
      satelliteFallbackLayer.coverage,
      satelliteFallbackLayer.loadedTiles,
      basemap,
    );
  }

  const { scaleX, scaleY } = drawLoadedTileLayer(
    ctx,
    mapRect,
    coverage,
    loadedTiles,
...
function drawBasemapAttribution(
  ...
  const basemapAttributionText = getBasemapAttributionText(basemap);
  const attributionText = [basemapAttributionText, dataAttributionText]
  ...
  const fontSize = Math.max(8, Math.min(11, Math.round(mapRect.width / 190)));
  ...
  const x = mapRect.x + mapRect.width - textWidth - 4;
  const y = mapRect.y + mapRect.height - textBlockHeight - 3;

  // Aucun cartouche, aucun halo et aucune ombre portée : uniquement le
  // texte d'attribution obligatoire. Sur les très grands canvas (notamment
  // l'export satellite haute définition), une ombre Canvas peut forcer une
  // surface intermédiaire supplémentaire et augmenter fortement la mémoire.
  // On l'interdit explicitement ici.
  ctx.fillStyle =
    basemap.id === "ign-satellite"
      ? "rgba(15, 23, 42, 1)"
      : "rgba(15, 23, 42, 0.50)";
...
function drawCreatorAttribution(
  ctx: CanvasRenderingContext2D,
  mapRect: ExportCanvasRect,
  attribution: DownloadCanvasExportInput["creatorAttribution"],
) {
  if (!attribution?.label.trim()) return;

  const label = attribution.label.trim();
  const fontSize = Math.max(10, Math.min(16, Math.round(mapRect.width / 95)));
  const paddingX = Math.max(7, Math.round(fontSize * 0.7));
  const paddingY = Math.max(5, Math.round(fontSize * 0.45));
  const margin = Math.max(8, Math.round(mapRect.width / 180));
  ...
  const right = attribution.position === "top-right" || attribution.position === "bottom-right";
  const bottom = attribution.position === "bottom-left" || attribution.position === "bottom-right";
  const hasFreePosition =
    attribution.mapPosition &&
    Number.isFinite(attribution.mapPosition.x) &&
    Number.isFinite(attribution.mapPosition.y);
  const freeX = hasFreePosition
    ? mapRect.x + Math.max(0, Math.min(1, attribution.mapPosition!.x)) * mapRect.width - width / 2
    : null;
  const freeY = hasFreePosition
    ? mapRect.y + Math.max(0, Math.min(1, attribution.mapPosition!.y)) * mapRect.height - height / 2
    : null;
  const x = freeX !== null
    ? Math.max(mapRect.x, Math.min(mapRect.x + mapRect.width - width, freeX))
    : right
      ? mapRect.x + mapRect.width - width - margin
      : mapRect.x + margin;
  // En bas à droite, laisser de la place aux crédits obligatoires du fond lorsque
  // l'ancien placement par coin est encore utilisé. Une position libre choisie
  // dans le rendu final est respectée exactement.
  const bottomOffset = !hasFreePosition && right && bottom ? Math.max(24, fontSize * 2.1) : 0;
  const y = freeY !== null
    ? Math.max(mapRect.y, Math.min(mapRect.y + mapRect.height - height, freeY))
    : bottom
      ? mapRect.y + mapRect.height - height - margin - bottomOffset
      : mapRect.y + margin;
...
function drawDromapGuestWatermark(
  ...
  ctx.textBaseline = "middle";
  ctx.lineJoin = "round";
  ctx.lineWidth = Math.max(1.25, fontSize * 0.07);

  // Watermark volontairement visible pour l'export sans compte, sans
  // cartouche ni flou coûteux. Le contour clair garantit la lisibilité sur
  // les fonds clairs comme sombres sans modifier le rendu cartographique.
...
function getCanvasFeatureVisualScale(
  input: DownloadCanvasExportInput,
  feature: DroMapFeature,
  currentZoom: number,
  fallbackReferenceZoom: number,
) {
  return input.useStoredFeatureVisualScale === true
    ? getFeatureVisualScaleForStaticRender(
        feature,
        currentZoom,
...
function drawFeature(
  ...
  if (geometry.type === "Point") {
    ...
    drawMarkerSymbolOnCanvasWithGroupOpacity(
      ctx,
      feature,
      canvasPoint.x,
      canvasPoint.y,
...
async function drawLegendSymbol(
  ctx: CanvasRenderingContext2D,
  feature: DroMapFeature,
  x: number,
  y: number,
  ...
  // Pour les zones, l’export rasterise désormais le même petit SVG que la
  // preview DOM au lieu de redessiner le figuré avec l’API Canvas. Les deux
  // rendus utilisent ainsi les mêmes règles de remplissage, contour, tirets,
  // hachures et points. Cela évite le liseré clair créé par l’anticrénelage
  // différent du Canvas entre le fond semi-transparent et le contour.
  ...
  const hasVisibleLegendZoneStroke =
    getZoneStrokeEnabled(feature) && getZoneVisibleStrokeOpacity(feature) > 0;
  const legendZoneDashStyle = symbolStyle?.dashStyle ?? getFeatureDashStyle(feature);
  const legendZoneDashLength = Math.max(1, symbolStyle?.dashLength ?? 14);
  const legendZoneDashGap = Math.max(1, symbolStyle?.dashGap ?? 7);
  const legendZoneDotGap = Math.max(4, symbolStyle?.dotSpacing ?? 7);
  const legendPatternInset = hasVisibleLegendZoneStroke ? zoneStrokeWidth / 2 : 0;
  ...
  const zoneDashArray =
    legendZoneDashStyle === "solid"
      ? ""
      : legendZoneDashStyle === "dotted"
        ? `0.01 ${legendZoneDotGap}`
        : `${legendZoneDashLength} ${legendZoneDashGap}`;

  const hatchElements: string[] = [];
  if (getZoneHatchingEnabled(feature)) {
    for (
      ...
      if (hatchingStyle === "horizontal") {
        hatchElements.push(
          `<line x1="${zoneX}" y1="${zoneY + offset}" x2="${zoneX + zoneWidth}" y2="${zoneY + offset}" />`,
        );
      } else if (hatchingStyle === "vertical") {
        hatchElements.push(
          `<line x1="${zoneX + offset}" y1="${zoneY}" x2="${zoneX + offset}" y2="${zoneY + zoneHeight}" />`,
        );
      } else if (hatchingStyle === "diagonal-left") {
        hatchElements.push(
          `<line x1="${zoneX + offset}" y1="${zoneY}" x2="${zoneX + offset + zoneHeight}" y2="${zoneY + zoneHeight}" />`,
        );
      } else {
        hatchElements.push(
          `<line x1="${zoneX + offset}" y1="${zoneY + zoneHeight}" x2="${zoneX + offset + zoneHeight}" y2="${zoneY}" />`,
        );
      }
    }
  }
  ...
  if (getZoneDotsEnabled(feature)) {
    ...
    for (let yDot = startY; yDot <= endY; yDot += dotsSpacing) {
      ...
      for (
        ...
        dotElements.push(
          `<circle cx="${xDot}" cy="${yDot}" r="${dotRadius}" />`,
        );
      }
    }
  }
  ...
  const svgMarkup = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${previewWidth * 4}" height="${previewHeight * 4}" viewBox="0 0 ${previewWidth} ${previewHeight}">
      <defs>
        <clipPath id="dromap-zone-symbol-clip">
          <rect
            x="${zoneX + legendPatternInset}"
            y="${zoneY + legendPatternInset}"
            width="${Math.max(0, zoneWidth - legendPatternInset * 2)}"
            height="${Math.max(0, zoneHeight - legendPatternInset * 2)}"
            rx="${Math.max(0, 2 - legendPatternInset / 2)}"
          />
        </clipPath>
      </defs>
      <rect
        ...
        height="${zoneHeight}"
        rx="2"
        fill="${legendFillOpacity > 0 ? escapeSvgAttribute(style.fillColor) : "none"}"
        fill-opacity="${legendFillOpacity > 0 ? Math.max(0.16, legendFillOpacity) : 0}"
        stroke="${hasVisibleLegendZoneStroke ? escapeSvgAttribute(style.color) : "none"}"
        stroke-opacity="${hasVisibleLegendZoneStroke ? getZoneVisibleStrokeOpacity(feature) : 0}"
        stroke-width="${hasVisibleLegendZoneStroke ? zoneStrokeWidth : 0}"
        ${hasVisibleLegendZoneStroke && zoneDashArray ? `stroke-dasharray="${zoneDashArray}"` : ""}
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      ${hatchElements.length > 0 ? `<g clip-path="url(#dromap-zone-symbol-clip)" stroke="${escapeSvgAttribute(getZoneHatchingColor(feature))}" stroke-width="${hatchWeight}" stroke-linecap="round" opacity="0.96">${hatchElements.join("")}</g>` : ""}
      ${dotElements.length > 0 ? `<g clip-path="url(#dromap-zone-symbol-clip)" fill="${escapeSvgAttribute(getZoneDotsColor(feature))}" opacity="0.98">${dotElements.join("")}</g>` : ""}
    </svg>
  `;

  const zoneImage = await loadImageFromBlob(
    new Blob([svgMarkup], { type: "image/svg+xml;charset=utf-8" }),
  );

  const targetWidth = previewWidth * previewScale;
  const targetHeight = previewHeight * previewScale;
  ctx.drawImage(
    zoneImage,
  ...
  for (const item of legendLayout.visibleItems) {
    ...
    textLines.forEach((line, lineIndex) => {
      ...
      if (legendPosition === "map") {
        ctx.save();
        ctx.lineJoin = "round";
        ctx.lineWidth = Math.max(2, legendLayout.itemFontSize * 0.16);
        ctx.strokeStyle = "rgba(255,255,255,0.94)";
        ctx.strokeText(line, textX, baseline);
        ctx.restore();
      }
      ctx.fillText(line, textX, baseline, textMaxWidth);
    });
  }
...
function createProjectJson(input: DownloadCanvasExportInput) {
  ...
  return {
    ...
    customMarkers: cloneJsonValue(
      input.customMarkers ??
        useEditorCustomMarkersStore.getState().customMarkers,
    ),
...
function parseDromapFeatureSource(
  ...
  const sourceLabel = getRecordValue(value, "sourceLabel");
  const sourceUrl = getRecordValue(value, "sourceUrl");
  const sourceLicense = getRecordValue(value, "sourceLicense");
  const sourceAttribution = getRecordValue(value, "sourceAttribution");
  const sourceVersion = getRecordValue(value, "sourceVersion");
  ...
  return {
    ...
    ...(typeof sourceLicense === "string" ? { sourceLicense } : {}),
...
export function parseDromapProjectJson(
  ...
  return {
    ...
    scaleBarSize: clamp(
      parseFiniteNumber(getRecordValue(safeScaleBar, "size"), 1),
      0.5,
      2,
    ),
    ...
    northArrowSize: clamp(
      parseFiniteNumber(getRecordValue(safeNorthArrow, "size"), 1),
      0.5,
      2,
    ),
...
async function renderCanvasExportToCanvas(
  ...
  const effectiveLegendPosition = hasVisibleLegend
    ...
    : "map";
  const shouldDrawLegendPanel = hasVisibleLegend;
  ...
  const layout = createExportLayout({
    ...
    legendFeaturesCount: hasVisibleLegendBody
      ...
      : 0,
    appearance,
    mapLegendContent: hasVisibleLegend
      ? {
  ...
  const scaleBarSize = Math.min(2, Math.max(0.5, input.scaleBarSize ?? 1));
  const northArrowSize = Math.min(2, Math.max(0.5, input.northArrowSize ?? 1));
  ...
  // dernière couche de l'export afin de ne jamais être masqués par un objet.
  drawBasemapAttribution(
    ctx,
    basemap,
    layout.mapRect,
    getDromapExportDataAttributionText(input.features, input.geoJsonLayers ?? []),
  );
  ...
  // Le crédit d'une carte publique copiée est la dernière couche visuelle.
  // S'il est obligatoire, son absence n'est jamais proposée par l'interface.
  drawCreatorAttribution(ctx, layout.mapRect, input.creatorAttribution);
...
export async function downloadCanvasExportAsPng(
  input: DownloadCanvasExportInput,
  options: { quality?: ExportVisualQuality; showDromapGuestWatermark?: boolean } = {},
) {
