...
export default function GeomanControls() {
  ...
  useEffect(() => {
    ...
    const onCreate = (event: GeomanLayerEvent) => {
      ...
      if (mode === "workspace-select") {
        if (
          ...
          useEditorWorkspaceStore.getState().setWorkspaceBounds(bounds);
    ...
    const container = map.getContainer();
    const WORKSPACE_TOOLTIP_FIRST = "Click pour poser 1er angle";
    const WORKSPACE_TOOLTIP_SECOND = "Click pour poser angle opposé";

    updateWorkspaceRectangleTooltip = () => {
      if (useEditorModeStore.getState().currentMode !== "workspace-select") {
        return;
      }
