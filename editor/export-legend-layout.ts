...
export type ExportLegendPositionForLayout = "left" | "right" | "top" | "bottom" | "map";
...
export function createExportLegendLayout({
  ...
  function getDisplayItemHeight(
    ...
    const textWidth = Number.isFinite(availableItemWidth)
      ? Math.max(1, Number(availableItemWidth) - labelOffset)
      : Math.max(1, legendRect.width - padding * 2 - labelOffset);
    const textLines = wrapLegendTextLines(
    ...
    return Math.max(
      symbolVisualHeight,
      Math.ceil(textLines.length * labelLineHeight) + 2,
    );
  }
  ...
  const isBottomSectionColumnMode =
    (legendPosition === "bottom" || legendPosition === "top") && hasExplicitSections;
  ...
  function buildLinearLayout(reserveWarningSpace: boolean) {
    ...
    const maxColumnsByWidth =
      (legendPosition === "bottom" || legendPosition === "top") ? getMaxColumnsByWidth(legendRect) : 1;
    ...
    if ((legendPosition === "bottom" || legendPosition === "top") && totalDisplayItems > 1) {
    ...
    for (
      ...
      const columnIndex =
        (legendPosition === "bottom" || legendPosition === "top") ? featureIndex % columnCount : 0;
      ...
      visibleItems.push({
        ...
        textLines:
          displayItem.type === "section"
            ...
            : wrapLegendTextLines(
                entry?.label ?? "Sans nom",
                Math.max(1, columnWidth - labelOffset),
                itemFontSize,
              ),
      });
