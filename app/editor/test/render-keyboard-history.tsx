...
function captureRenderSnapshot(): RenderHistorySnapshot {
  const exportState = useEditorTestExportStore.getState() as unknown as Record<
    string,
    unknown
  >;
...
function restoreRenderSnapshot(snapshot: RenderHistorySnapshot) {
  (
    useEditorTestExportStore.setState as unknown as (
      partial: SerializableState,
    ) => void
  )(cloneSerializableValue(snapshot.exportState) as SerializableState);

  (
    useEditorTestMapLabelsStore.setState as unknown as (
      partial: SerializableState,
    ) => void
...
export function RenderKeyboardHistory() {
  ...
  useEffect(() => {
    ...
    const unsubscribeExport = useEditorTestExportStore.subscribe(handleStoreChange);
    const unsubscribeLabels = useEditorTestMapLabelsStore.subscribe(handleStoreChange);
