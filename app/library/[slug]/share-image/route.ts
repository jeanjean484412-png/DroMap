...
export async function GET(
  ...
  try {
    const response = await publicationsAdminFetch(
      `/dromap_publications?slug=eq.${encodeURIComponent(slug)}&select=thumbnail_data_url&limit=1`,
      { method: "GET" },
    );
    const rows = response.ok ? await parseJsonResponse<DromapPublicationRow[]>(response) : [];
    const image = await decodeSupportedImageDataUrl(rows?.[0]?.thumbnail_data_url);
