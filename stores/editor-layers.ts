...
import { registerEditorHistoryParticipant } from "@/stores/editor-history-coordinator";
...
function normalizeLayer(layer: Partial<DroMapLayer>, index: number): DroMapLayer {
  ...
  return {
    ...
    ...(typeof layer.sourceSavedLayerId === "string" && layer.sourceSavedLayerId.trim()
      ...
      : {}),
    ...(typeof layer.sourceGeoJsonLayerId === "string" && layer.sourceGeoJsonLayerId.trim()
      ? { sourceGeoJsonLayerId: layer.sourceGeoJsonLayerId.trim() }
      : {}),
    ...(typeof layer.sourceGeoJsonLayerName === "string" && layer.sourceGeoJsonLayerName.trim()
      ? { sourceGeoJsonLayerName: layer.sourceGeoJsonLayerName.trim() }
      : {}),
    ...(typeof layer.sourceGeoJsonSourceName === "string"
      ? { sourceGeoJsonSourceName: layer.sourceGeoJsonSourceName }
      : layer.sourceGeoJsonSourceName === null
        ? { sourceGeoJsonSourceName: null }
        : {}),
  };
}
...
export const useEditorLayersStore = create<EditorLayersState>((set, get) => ({
...
registerEditorHistoryParticipant("drawing-layers", {
  capture: () => {
    const state = useEditorLayersStore.getState();
    return {
      layers: cloneJsonValue(state.layers),
      activeLayerId: state.activeLayerId,
    };
  },
  restore: (snapshot) => {
    if (!snapshot || typeof snapshot !== "object") return;
    const value = snapshot as { layers?: unknown; activeLayerId?: unknown };
    const layers = Array.isArray(value.layers) ? value.layers : [];
    const activeLayerId =
      typeof value.activeLayerId === "string" ? value.activeLayerId : null;
    useEditorLayersStore.getState().setLayers(
      layers as DroMapLayer[],
      activeLayerId,
    );
  },
});
