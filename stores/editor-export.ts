...
export type ExportLegendPosition = "right" | "left" | "top" | "bottom" | "map";
...
type EditorExportState = {
  ...
  setScaleBarSize: (size: number) => void;
...
export const useEditorExportStore = create<EditorExportState>(
  (set) => ({
    ...
    setScaleBarSize: (size) => {
      set({ scaleBarSize: Math.min(2, Math.max(0.5, size)) });
    },
    ...
    setNorthArrowSize: (size) => {
      set({ northArrowSize: Math.min(2, Math.max(0.5, size)) });
    },
